...
export default function (knowledgeBaseId?: string) {
  ...
  const getKnowled = (
    ...
    const cardList_ = data.map((item: any) => {
      const rawName = item.file_name || item.title || item.source || /* 원문: 未命名文档 */ '이름 없는 문서'
  ...
  const delKnowledge = (index: number, item: any, onSuccess?: () => void) => {
    ...
    return delKnowledgeDetails(item.id)
      .then((result: any) => {
        if (result.success) {
          MessagePlugin.info(/* 원문: 知识删除成功！ */ "지식 삭제 성공!");
          if (onSuccess) {
            onSuccess();
          } else {
            getKnowled();
          ...
          MessagePlugin.error(/* 원문: 知识删除失败！ */ "지식 삭제 실패!");
          return false;
        }
      })
      .catch(() => {
  ...
  const requestMethod = (file: any, uploadInput: any) => {
    ...
    if (!currentKbId) {
      MessagePlugin.error(/* 원문: 缺少知识库ID */ "지식 베이스 ID가 없습니다");
      return;
    }
    ...
    uploadKnowledgeFile(currentKbId, { file, tag_id: tagIdToUpload })
      .then((result: any) => {
        if (result.success) {
          MessagePlugin.info(/* 원문: 上传成功！ */ "업로드 성공!");
          getKnowled({ page: 1, page_size: 35 }, currentKbId);
        } else {
          const errorMessage = result.error?.message || result.message || "업로드에 실패했습니다!";
          MessagePlugin.error(result.code === 'duplicate_file' ? "파일이 이미 존재합니다" : errorMessage);
        }
        uploadInput.value.value = "";
      })
      .catch((err: any) => {
        const errorMessage = err.error?.message || err.message || "업로드에 실패했습니다!";
    ...
    getKnowledgeDetails(item.id)
      .then((result: any) => {
        if (result.success && result.data) {
          const { data } = result;
          Object.assign(details, {
            title: data.file_name || data.title || data.source || /* 원문: 未命名文档 */ '이름 없는 문서',
