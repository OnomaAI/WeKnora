...
5. **Key Characteristics**: What notable features does the data have? (e.g., contains geographic locations, has categorical labels, hierarchical relationships, etc.)

**Important Notes**:
- Do not output specific data values or sample content
- Use a generalized description so users can quickly determine whether the table contains the information they need
...
// - 如果从样本数据中能推断出枚举值范围，可以概括说明（如：状态字段包含待处理/进行中/已完成等状态）`
	columnDescriptionsPromptTemplate = `You are a data analysis expert. Based on the following table structure information and data samples, generate structured descriptive information for each column.
...
3. **Business Purpose**: The role of this field in the business (e.g., used for user identification, amount calculation, time sorting, etc.)
4. **Data Characteristics**: Notable features of the data (e.g., unique identifier, nullable, has enumerated values, has units, etc.)
...
- Use clear business terminology for easy understanding and search  
- If the range of enumerated values can be inferred from the sample data, summarize them (e.g., status field includes pending / in progress / completed)
