...
type QueryKnowledgeGraphInput struct {
	...
	Query string `json:"query" jsonschema:"Query text (entity name or search text)"`
}
...
func (t *QueryKnowledgeGraphTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	for _, kbID := range input.KnowledgeBaseIDs {
		...
		go func(id string) {
			...
			if kb.ExtractConfig == nil || (len(kb.ExtractConfig.Nodes) == 0 && len(kb.ExtractConfig.Relations) == 0) {
				...
				kbResults[id] = &graphQueryResult{kbID: id, err: fmt.Errorf("knowledge graph extraction is not configured")}
				mu.Unlock()
				return
			}
	...
	if len(allResults) == 0 {
		return &types.ToolResult{
			Success: true,
			// Output:  "未找到相关的图谱信息。",
			Output: "No relevant graph information was found.",
	...
	output += fmt.Sprintf("🎯 Target knowledge base(s): %v\n", input.KnowledgeBaseIDs)
	// output += fmt.Sprintf("✓ 找到 %d 条相关结果（已去重）\n\n", len(allResults))
	output += fmt.Sprintf("✓ Found %d relevant result(s) (deduplicated)\n\n", len(allResults))
	...
	for kbID, config := range graphConfigs {
		...
		if len(nodes) > 0 {
			...
			// output += "  ⚠️ 未配置实体类型\n"
			output += "  ⚠️ Entity types are not configured\n"
		}
		...
		if len(relations) > 0 {
			...
			// output += "  ⚠️ 未配置关系类型\n"
			output += "  ⚠️ Relation types are not configured\n"
		}
		output += "\n"
	}
	...
	if len(kbCounts) > 0 {
		...
		for kbID, count := range kbCounts {
			...
			output += fmt.Sprintf("  - %s: %d result(s)\n", kbID, count)
		}
		output += "\n"
	}
	...
	if !hasGraphConfig {
		...
		output += "💡 Returning relevant document chunks (graph not configured for KB)\n\n"
	} else {
		// output += "💡 基于图谱配置的相关内容检索\n\n"
		output += "💡 Relevant content retrieval based on graph configuration\n\n"
	}
	...
	output += "- ✓ Use list_knowledge_chunks to explore surrounding context\n"
	if !hasGraphConfig {
