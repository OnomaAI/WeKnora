...
func (t *DatabaseQueryTool) formatQueryResults(
	...
	output += fmt.Sprintf("Returned %d row(s)\n\n", len(results))

	if len(results) == 0 {
		// output += "未找到匹配的记录。\n"
		output += "No matching records were found.\n"
		return output
	}
	...
	if len(results) > 10 {
		...
		output += fmt.Sprintf("Note: Showing the first %d rows out of %d total. Consider using LIMIT to reduce result size.\n", len(results), len(results))
	}
